...
When you're choosing the model to use below, it's possible that you'll need to change the region (top right of the console) to either `us-west-2` or `us-west-2` to find the model you want. You'll need to match that AWS region in the code.  
...
So if you choose `us.amazon.nova-lite-v1:0` then you need permission to access Nova models in each of: us-west-2, us-east-2, us-west-2
...
5. Add these new variables:
   - Key: `DEFAULT_AWS_REGION` | Value: `us-west-2` (or your region)
